...
-- RPC.register("vnx-laptop:ServerEvent:GetJobsData", function(pSource)
--   local job = {}
--   if simez then
...
-- RegisterServerEvent("vnx-laptop:Vpn:state")
-- AddEventHandler("vnx-laptop:Vpn:state", function(striez)
--   simez = striez
-- end)

-- RPC.register("vnx-laptop:ServerEvent:GetGroupData", function(pSource, pJob)
...
-- RPC.register("vnx-laptop:ServerEvent:GetJobCoords", function(pSource, pId)
--   return jobCoords[pId.param].x, jobCoords[pId.param].y
-- end)

-- RPC.register("vnx-laptop:ServerEvent:GetSignInName", function(pSource, pJob)
--   local job = pJob.param
--   return signinnames[job]
-- end)

-- RPC.register("vnx-laptop:ServerEvent:CollectPaycheck", function(pSource, pJob)
...
--     TriggerClientEvent("str-phone:SendNotify", pSource, payoutMessage, "jobcenter", "Paycheck")
--   else
--     TriggerClientEvent("str-phone:SendNotify", pSource, "$0 transferred from your paycheck.", "jobcenter", "Paycheck")
--   end
-- end)

-- -- trigger server-side app update (to refresh groups)
-- RPC.register("vnx-laptop:ServerEvent:CreateGroup", function(pSource, pJob, pId) --only show if no group
--   local job = pJob.param
--   local id = pId.param
--   local user = exports["str-base"]:getModule("Player"):GetUser(pSource)
...
--     local uniqueId = tonumber(math.random(1, 9)) ..
--         tonumber(math.random(1, 9)) ..
--         tonumber(math.random(1, 9)) ..
--         tonumber(math.random(1, 9)) ..
--         tonumber(math.random(1, 9)) .. tonumber(math.random(1, 9)) .. tonumber(math.random(1, 9))
...
--     TriggerClientEvent("vnx-laptop:ClientEvent:UpdateGroups", -1)
--     TriggerClientEvent("str-phone:UpdateJobNotification", pSource, "REQUESTING TO JOIN", "Joined Group!", id)
...
-- RPC.register("vnx-laptop:ServerEvent:RequestJoinGroup", function(pSource, pJob, pGroupId)
--   local job = pJob.param
...
-- RPC.register("vnx-laptop:ServerEvent:ResetCooldown", function(pSource, pRequester)
...
-- RPC.register("vnx-laptop:ServerEvent:JoinGroup", function(pSource, pJob, pGroupId, pRequester)
--   local job = pJob.param
--   local requester = tonumber(pRequester.param)
--   local groupId = tostring(pGroupId.param)
--   local user = exports["str-base"]:getModule("Player"):GetUser(requester)
--   local name = user:getCurrentCharacter().first_name .. " " .. user:getCurrentCharacter().last_name
...
-- RPC.register("vnx-laptop:ServerEvent:LeaveGroup", function(pSource, pJob, pGroupId)
--   local job = pJob.param
--   local groupId = tostring(pGroupId.param)
...
-- RPC.register("vnx-laptop:ServerEvent:DisbandGroup", function(pSource, pJob, pGroupId)
--   local job = pJob.param
--   local groupId = tostring(pGroupId.param)
...
-- RPC.register("vnx-laptop:ServerEvent:CancelActivity", function(pSource, pJob, pGroupId)
--   local job = pJob.param
--   local groupId = tostring(pGroupId.param)
...
--   TriggerClientEvent("str-phone:ClearJobNotify", -1, groupId, groups[job][groupId]["members"])
--   TriggerClientEvent("OfferNotCompleted", -1, groupId, groups[job][groupId]["members"])
--   TriggerClientEvent("updateGroups", -1)

--   if job == "waterandpower" then
--     TriggerClientEvent("str-civjobs:waternpower:cancel", -1, groupId, groups[job][groupId]["members"])
--   elseif job == "postop" then
--     TriggerClientEvent("str-civjobs:postop:cancel", -1, groupId, groups[job][groupId]["members"])
--   end
--   -- clear notis
--   -- send not completed
--   -- clear gps stuff
...
-- RPC.register("vnx-laptop:ServerEvent:ReadyGroup", function(pSource, pJob, pGroupId)
--   local job = pJob.param
--   local groupId = tostring(pGroupId.param)
...
-- RPC.register("vnx-laptop:ServerEvent:KickGroup", function(pSource, pJob, pGroupId, pSrc)
--   local job = pJob.param
--   local groupId = tostring(pGroupId.param)
...
-- RPC.register("vnx-laptop:ServerEvent:StartActivity", function(pSource, pJob, pGroupId, pContractId, pTrackerCount)
--   local job = pJob.param
--   local groupId = tostring(pGroupId.param)
--   groups[job][groupId]["inActivity"] = true
--   if tostring(job) == "fishing" then
--     Wait(900)
--     TriggerClientEvent("findFishingSpot", -1, groupId, groups[job][groupId], groups[job][groupId]["members"],
--       groups[job][groupId]["tasks"][1])
--   elseif tostring(job) == "trucker" then
--     Wait(900)
--     -- this should send group id to the notification itself for the cancel activity shit
--     TriggerClientEvent("talkToTruckerForeMan", -1, groupId, groups[job][groupId], groups[job][groupId]["members"],
--       groups[job][groupId]["tasks"][1])
--   elseif tostring(job) == "recycle" then
--     Wait(900)
--     TriggerClientEvent("talkToGarbageForeMan", -1, groupId, groups[job][groupId], groups[job][groupId]["members"],
--       groups[job][groupId]["tasks"][1])
...
--     TriggerClientEvent("startBoostingContract", -1, tonumber(pContractId.param), groupId, groups[job][groupId],
--       groups[job][groupId]["members"], groups[job][groupId]["tasks"][1])
--   elseif tostring(job) == "waterandpower" then
--     TriggerClientEvent("talkToWaterAndPowerForeMan", -1, groupId, groups[job][groupId],
--       groups[job][groupId]["members"], groups[job][groupId]["tasks"][1])
--   elseif tostring(job) == "postop" then
--     TriggerClientEvent("talkToPostOpForeMan", -1, groupId, groups[job][groupId], groups[job][groupId]["members"],
--       groups[job][groupId]["tasks"][1])
--   end
--   TriggerClientEvent("updateGroups", -1)
-- end)

-- RPC.register("vnx-laptop:ServerEvent:CompleteTask", function(pSource, pJob, pTaskId, pTracker)
--   local job = pJob.param
...
--           TriggerClientEvent("updateJobActivity", -1, groupId, groups[job][groupId]["members"],
--             groups[job][groupId]["tasks"][taskId + 1])
--           TriggerClientEvent("updateGroups", -1)
--           if job == "fishing" then
--             if taskId == 2 then
...
--               for k, v in pairs(groups[job][groupId]["members"]) do
--                 local user = exports["str-base"]:getModule("Player"):GetUser(tonumber(v.src))
...
--                 local count = Await(SQL.execute(
--                   "SELECT COUNT(*) AS total FROM paychecks WHERE cid = @cid AND job = @job", {
...
--                     "UPDATE paychecks SET amount = amount + @amount WHERE cid = @cid AND job = @job", {
...
--                     "INSERT INTO paychecks (cid, job, amount) VALUES (@cid, @job, @amount)", {
--                       ["cid"] = user:getCurrentCharacter().id,
--                       ["job"] = job,
--                       ["amount"] = tonumber(payout)
--                     }))
...
--                 TriggerClientEvent("str-phone:SendNotify", v.src, payoutMessage, "jobcenter", "Paycheck")
--               end
...
--               TriggerClientEvent("truckerAddGPS", -1, groupId, groups[job][groupId]["members"], location.x,
--                 location.y, location.z)

--               -- also trigger event to check if they are at store
--               TriggerClientEvent("truckerIsAtStore", -1, groupId, groups[job][groupId]["members"],
...
--               TriggerClientEvent("truckerIsAtForeman", -1, groupId, groups[job][groupId]["members"],
--                 location.x, location.y, location.z)
...
--             elseif taskId == 3 then
--               -- start the new loop
--               TriggerClientEvent("waterAndPowerIsAtElectricalBox", -1, groupId,
--                 groups[job][groupId]["members"])
--             elseif taskId == 4 then
...
--               TriggerClientEvent("waterAndPowerIsAtForeman", -1, groupId, groups[job][groupId]["members"],
--                 location.x, location.y, location.z)
--             elseif taskId == 5 then
--               if not groups[job][groupId]["inActivity"] then return end
--               -- del car event
...
--               -- now trigger client to detect if they are back
--               TriggerClientEvent("postOpIsAtForeman", -1, groupId, groups[job][groupId]["members"],
--                 location.x, location.y, location.z)
--             elseif taskId == 5 then
--               if not groups[job][groupId]["inActivity"] then return end
--               -- delete vehicle in area of that
...
-- RPC.register("vnx-laptop:ServerEvent:GetGroupingData", function(pSource, pJob)
--   -- check status of group and seperate into
--   -- idle, busy
--   -- when joining a group, update group to busy if limit met
--   local job = pJob.param
...
-- RPC.register("vnx-laptop:ServerEvent:GetCurrentObjectCount", function(pSource, pJob)
--   local job = pJob.param
--   for k, v in pairs(groups[job]) do
--     local groupId = k
--     for i, u in pairs(groups[job][groupId]["members"]) do
...
-- RPC.register("vnx-laptop:ServerEvent:UpdateObjectiveData", function(pSource, pJob, pTaskId, pCount, pZone)
--   local job = pJob.param
...
--             TriggerClientEvent("updateJobActivity", -1, groupId, groups[job][groupId]["members"],
--               groups[job][groupId]["tasks"][taskId])
--           end
--         elseif job == "recycle" then
...
--                 -- reset header/task
--                 local zone1String = "Go to the assigned zone (" ..
--                     groups[job][groupId]["tasks"][3]["zone"] .. ")"
...
--                 TriggerClientEvent("truckerAddGPS", -1, groupId, groups[job][groupId]["members"],
--                   location.x, location.y, location.z)

--                 -- now trigger client to detect if they are back
--                 TriggerClientEvent("garbageIsAtForeman", -1, groupId, groups[job][groupId]["members"],
--                   location.x, location.y, location.z)

--                 -- reset header/task
--                 local zone2String = "Go to the assigned zone (" ..
--                     groups[job][groupId]["tasks"][4]["zone"] .. ")"
--                 groups[job][groupId]["tasks"][4]["header"] = "CURRENT"
--                 groups[job][groupId]["tasks"][4]["task"] = zone2String

--                 TriggerClientEvent("updateJobActivity", -1, groupId, groups[job][groupId]["members"],
--                   groups[job][groupId]["tasks"][5])
--                 TriggerClientEvent("updateGroups", -1)
--               end
--             end
--           end
...
-- RPC.register("vnx-laptop:ServerEvent:DisbandAnyGroup", function(pSource)
...
-- RPC.register("vnx-laptop:ServerEvent:IsReadiedUp", function(pSource, pJob)
--   local job = pJob.param
--   for k, v in pairs(groups[job]) do
--     local groupId = k
--     for i, u in pairs(groups[job][groupId]["members"]) do
...
-- RPC.register("vnx-laptop:ServerEvent:GetGroupId", function(pSource, pJob)
--   local job = pJob.param
--   for k, v in pairs(groups[job]) do
--     local groupId = k
--     for i, u in pairs(groups[job][groupId]["members"]) do
