...
CustomMinigame = function(typeContract)
  local success = nil
  ...
  exports['str-hackdevice']:OpenDevice(function(OpenDeviceResult)
    onComplete(OpenDeviceResult)
  end, Config.NumberOfHacks[typeContract], Config.TimeOfHacksByContractType[typeContract])
  ...
  -- local game = exports['str-hackdevice']:OpenDevice()
end
...
CustomMinigameHack = function() --The minigame will start when you get in car, [NEEDS TO RETURN TRUE OR FALSE]
  ...
  if types == 1 then
    return exports["str-minigame"]:Open()
  elseif types == 2 then
    return exports["str-minigame"]:Open()
  elseif types == 3 then
    return exports["str-minigame"]:Open()
...
function InCoolDown()
  local result = RPC.execute("vnx-laptop:ServerEvent:Boosting:CoolDown")

  print("InCoolDown=>Client:" .. json.encode(result))
